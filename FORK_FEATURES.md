...
| `chat-timeout-msg` | Blank-exception-to-human-readable conversion, streamed timeout/stream-stall messaging, header cleanup, first-meaningful-chunk idle-timeout skip, and `fetch_url` Web Loader timeout fallback | `fork:chat-timeout-msg` | [test_chat_timeout_msg_wiring.py](backend/open_webui/test/util/test_chat_timeout_msg_wiring.py) |
