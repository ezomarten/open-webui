...
Fork mainline now tracks upstream `v0.9.1`. The current published GHCR release is `0.9.1-publicshare.3`.
...
```bash
docker pull ghcr.io/farefore/open-webui-public-share:0.9.1-publicshare.3
```
...
To run the current published fork image instead of a local build, set `OPENWEBUI_IMAGE=ghcr.io/farefore/open-webui-public-share:0.9.1-publicshare.3`.
