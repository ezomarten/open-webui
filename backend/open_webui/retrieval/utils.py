...
def get_loader(request, url: str, config: dict, timeout: Optional[float] = None):  # fork:chat-timeout-msg
...
def _get_content_from_url_sync(request, url: str, loader_config):
    ...
    if response is None or _is_text_content_type(content_type):
        if response is not None:
            response.close()
        loader = get_loader(request, url, loader_config, timeout=loader_timeout)  # fork:chat-timeout-msg
        docs = loader.load()
        content = ' '.join([doc.page_content for doc in docs])
        return content, docs
