...
DEFAULT_WEB_LOADER_TIMEOUT_SECONDS = 30.0  # fork:chat-timeout-msg
...
def get_loader(request, url: str, timeout: Optional[float] = None):  # fork:chat-timeout-msg
...
def get_content_from_url(request, url: str) -> str:
    ...
    loader_timeout = get_web_loader_timeout_seconds(request)  # fork:chat-timeout-msg
    try:
        response = requests.get(
            url,
            stream=True,
            timeout=loader_timeout,  # fork:chat-timeout-msg
    ...
    if response is None or _is_text_content_type(content_type):
        if response is not None:
            response.close()
        loader = get_loader(request, url, timeout=loader_timeout)  # fork:chat-timeout-msg
        docs = loader.load()
        content = ' '.join([doc.page_content for doc in docs])
        return content, docs
