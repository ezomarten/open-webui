...
from starlette.datastructures import Headers, MutableHeaders  # fork:public-share
...
from open_webui.config import (
    ...
    ENABLE_PUBLIC_CHAT_SHARING,  # fork:public-link-settings
    ...
    WEBUI_NAME,
    WEBUI_URL,
    PUBLIC_SHARE_BASE_URL,  # fork:public-link-settings
    WHISPER_LANGUAGE,
    WHISPER_MODEL,
...
from open_webui.routers import (
    ...
    pipelines,
    prompts,
    public_shares,  # fork:public-share
    retrieval,
    scim,
    skills,
    tasks,
...
from open_webui.utils.public_share import (  # fork:public-share
...
app.state.config.ENABLE_PUBLIC_CHAT_SHARING = ENABLE_PUBLIC_CHAT_SHARING  # fork:public-link-settings
app.state.config.PUBLIC_SHARE_BASE_URL = PUBLIC_SHARE_BASE_URL  # fork:public-link-settings
...
# fork:public-share — must be defined before app.add_middleware call below
class PublicShareHostMiddleware:
    ...
    async def __call__(self, scope, receive, send) -> None:
        ...
        public_share_api_remainder = (
            request_path[len(public_share_api_prefix) :].strip('/')
            if request_path.startswith(public_share_api_prefix)
            else ''
        )
        ...
        public_share_enabled = is_public_share_enabled(
            self._fastapi_app.state.config.ENABLE_PUBLIC_CHAT_SHARING,
            self._fastapi_app.state.PUBLIC_SHARE_BASE_URL,
        )
...
app.add_middleware(PublicShareHostMiddleware, fastapi_app=app)  # fork:public-share
...
app.include_router(public_shares.router, prefix='/api/v1/public-shares', tags=['public-shares'])  # fork:public-share
            ...
            **(
                {
                    ...
                    'enable_public_chat_sharing': is_public_share_enabled(  # fork:public-link-settings
