...
def test_get_exception_message_uses_http_exception_detail():
    ...
    assert error_handling.get_exception_message(exc) == "Open WebUI: Server Connection Error"


def test_get_exception_message_formats_timeout_error():
    assert (
        error_handling.get_exception_message(asyncio.TimeoutError(), request_timeout_seconds=300)
...
def test_get_exception_message_preserves_explicit_timeout_message():
    exc = asyncio.TimeoutError("Upstream streaming response stalled for 300 seconds without receiving data.")
