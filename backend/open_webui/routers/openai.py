...
async def get_models_request(
    ...
    response = await send_get_request(  # fork:openrouter-zdr
        request,
    ...
    return normalize_models_response(url, api_config, response)  # fork:openrouter-zdr
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                ...
                if api_config.get('azure') or api_config.get('provider') == 'azure':
                    ...
                    async with session.get(
                        get_models_list_url(url, api_config),  # fork:openrouter-zdr
                        headers=headers,
                        cookies=cookies,
                        ssl=AIOHTTP_CLIENT_SESSION_SSL,
                    ) as r:
                        ...
                        if is_openrouter_zdr_model_list_enabled(url, api_config):  # fork:openrouter-zdr
...
async def generate_chat_completion(
    ...
    payload = apply_openrouter_zdr_preferences(url, api_config, payload)  # fork:openrouter-zdr
...
async def responses(
    ...
    payload = apply_openrouter_zdr_preferences(url, api_config, payload)  # fork:openrouter-zdr
    payload['model'] = strip_provider_model_prefix(payload['model'], api_config.get('prefix_id'))
    body = JSONCodec.dumps(payload)  # regenerate after ZDR patch
...
async def proxy(path: str, request: Request, user=Depends(get_verified_user)):
    ...
    if isinstance(payload, dict):  # fork:openrouter-zdr
